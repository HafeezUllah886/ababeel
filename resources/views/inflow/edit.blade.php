        ...
        <div class="card">
            <div class="card-body">
                <div class="card-title d-flex justify-content-between">
                    <h5>Update Receiving Vouchare</h5>
                </div>
                    ...
                    <div class="row">
                                    ...
                                    <select id="normalize" name="supplier">
                                        <option value=""></option>
                                         @foreach ($suppliers as $supplier)
                                             <option {{ $supplier->id == $inflow->supplier->id ? 'selected' : '' }}  value="{{ $supplier->id }}">{{ $supplier->title }}</option>
                                         @endforeach
                                      </select>
                                      @error('supplier')
                        ...
                        <div class="col-md-6">
                            <div class="form-group mt-3">
                                ...
                                <input type="date" name="date" value="{{ old('date', $inflow->date) }}" class="form-control" id="date">
                                @error('date')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mt-3">
                               ...
                               <Select name="isPaid" class="form-control">
                                <option {{ $inflow->isPaid == 'yes' ? 'selected' : '' }} value="yes">Yes</option>
                                <option {{ $inflow->isPaid == 'no' ? 'selected' : '' }} value="no">No</option>
                               </Select>
                                @error('isPaid')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                               ...
                               <Select name="paidFrom" class="form-control">
                                <option value="">Select</option>
                                    @foreach ($accounts as $account)
                                        <option {{ $account->id == @$inflow->account->id ? 'selected' : '' }} value="{{ $account->id }}">{{ $account->title }}</option>
                                    @endforeach
                               </Select>
                                @error('paidFrom')
                                    <span class="text-danger">{{ $message }}</span>
