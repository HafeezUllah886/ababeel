                    ...
                    <table id="zero-config" class="table dt-table-hover" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $vouchar->id }}</td>
                                <td>{{ $vouchar->supplier->title }}</td>
                                <td>{{ date("d-m-Y", strtotime($vouchar->date)); }}</td>
                                <td>{{ $vouchar->isPaid }}</td>
                                <td>{{ @$vouchar->account->title }}</td>
                                <td>
                                    <a href="{{url('/inflow/edit/')}}/{{$vouchar->id}}" class="text-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-3"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></a>
                                    / <a href="{{url('/receiving/add_items/')}}/{{$vouchar->id}}" class="text-success">Open</a>
                                </td>
                            </tr>
                            @endforeach
